...
| ◐ | lib/nvmf/auth.c | 2026-05-07 **부분 완료** (병렬 agent, 토큰 한도) — 원본 933 → 1459줄 (+526). 한국어 주석 92. 4섹션 상단 블록 + DH-HMAC-CHAP 4단계 시퀀스(negotiate/challenge/reply/success1/2) + 일부 함수 §2 보강. **잔여**: PSK in-band 경로, 일부 timeout/abort 처리. |
| ◐ | lib/nvmf/transport.c | 2026-05-07 **부분 완료** (병렬 agent, 토큰 한도) — 원본 1086 → 1527줄 (+441). 한국어 주석 123. 4섹션 상단 블록 + transport_create/destroy/register/listener/request pool 핵심 함수 §2. **잔여**: subsystem listener 동기화, get_transport 열거. |
| ◐ | lib/nvmf/ctrlr_bdev.c | 2026-05-07 **부분 완료** (병렬 agent, 토큰 한도) — 원본 1134 → 1548줄 (+414). 한국어 주석 107. 4섹션 상단 블록 + read/write/compare/dataset_management/flush 등 핵심 opcode dispatch §2. **잔여**: ZNS/PR/abort/DIF·PI 일부, completion → CQE 변환 인라인. |
