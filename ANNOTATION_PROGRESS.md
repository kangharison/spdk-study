...
**[추가] 미착수 큰 파일 우선순위 (2026-05-28 surveys)**:
- ☑ `lib/nvmf/rdma.c` (5403) — Part 1~5 완료 (구조체 + state dispatcher + qpair lifecycle + resources + accept + CM event + SGL parsing + connect 협상 + QP create + disconnect + vtable). 사실상 완결. 잔여 보조 함수 (setup_request/fill_wr_sgl/fused/dump_opts/listen/discover) 는 별도 세션. **다음 권장**: lib/nvmf/vfio_user.c (5076) 또는 lib/nvmf/fc.c (4013).
- ◐ `lib/nvmf/vfio_user.c` (현재 7082줄, 마커 618) — 상단 블록 + vtable 콜백 + create/destroy/stop_listen 완료. **잔여 = 707~4500줄 hot-path 내부 헬퍼 약 60개**(q_addr, queue_index, sq_headp/cq_tailp, sq_head_advance/cq_tail_advance, io_q_exists, ctrlr_id/endpoint_id 등)의 §2 헤더 + 본문 인라인 → batch4(2026-06-04)에서 이어감.
- ☑ `lib/ublk/ublk.c` (현재 3829줄, 마커 625) — **2026-06-04 batch2 완료**. start_disk/start_dev/queue_io_init/recovery/ctrl 함수 포함 전체.
- ◐ `lib/nvmf/fc.c` (현재 5049줄, 마커 740) — **2026-06-04 batch2 절반 완료** (상단~nvmf_fc_hwqp_process_pending_reqs ~2615줄). **잔여 = 2621줄 이후**: LS pending 재시도, poll group create/destroy/poll, 트랜스포트 ops(create/destroy/listen/stop_listen/accept), association/connection lifecycle, HW port adm 함수군 → batch4(2026-06-04)에서 이어감.
- ✂ `lib/scsi/scsi_bdev.c` (2231) — SCSI 관련, 사용자 작업 대상 외(제외)
- ☑ `lib/nvmf/fc_ls.c` (현재 2610줄, 마커 689) — **2026-06-04 batch2 완료**. FC Link Service 전체.
- ☑ `lib/vmd/vmd.c` (현재 2778줄, 마커 638) — **2026-06-04 batch2 완료**. Intel VMD enumerate/hotplug/MSI-X, 45개 함수 전체.
- ☑ `lib/mlx5/mlx5_umr.c` (현재 2274줄, 마커 589) — **2026-06-04 batch2 완료**. UMR MR/MKEY 등록.
- ✂ `lib/scsi/scsi_pr.c` (1055) — SCSI 관련, 사용자 작업 대상 외(제외)

**[2026-06-04 병렬 에이전트 세션 추가 완료]**:
- ☑ `lib/vmd/led.c` (331줄, 마커 48) — VMD 슬롯 LED 제어, batch2 검수+갭 보강 완료.
- ☑ `module/bdev/gpt/vbdev_gpt.c` (1083줄, 마커 330) — GPT 파티션 가상 bdev, batch2 완료.
- ☑ `module/bdev/raid/raid5f.c` (2319줄, 마커 460) — RAID5F full-stripe write, batch2 완료.
- ☑ `lib/env_dpdk/pci.c` (2588줄, 마커 470) — DPDK PCI 추상화, batch2 완료.
- ☑ `lib/env_dpdk/memory.c` (3462줄, 마커 608) — DPDK 메모리/DMA vtophys, batch2 완료.
- **batch3 완료(2026-06-04)**:
  - ☑ `module/bdev/raid/bdev_raid.c` (6422줄, 마커 650) — RAID 코어 전체.
  - ☑ `module/sock/posix/posix.c` (3437줄, 마커 867) — POSIX/SSL sock 백엔드 전체.
  - ☑ `module/bdev/lvol/vbdev_lvol.c` (3980줄, 마커 522) — lvol bdev 전체.
  - ◐ `module/bdev/nvme/bdev_nvme.c` (11096줄, 마커 492) — bdev_nvme_destruct까지. **잔여 = bdev_nvme_create_qpair(~3460줄) 이후 ~7600줄** → batch5에서 이어감.
  - ◐ `lib/bdev/bdev.c` (13874줄, 마커 872) — bdev_qos_config_json까지. **잔여 = spdk_bdev_subsystem_config_json(~4267줄) 이후** → batch5에서 이어감.
- **batch4 완료(2026-06-04)**:
  - ☑ `lib/nvmf/fc.c` (5857줄, 마커 1485) — **FC target 전체 완료** (batch2 절반 + batch4 나머지).
  - ☑ `lib/ftl/ftl_core.c` (1562줄, 마커 222) — FTL 코어, 검수+갭 보강 완료.
  - ☑ `lib/thread/thread.c` (5135줄, 마커 755) — thread/poller/reactor 코어, 검수+보강 완료.
  - ◐ `lib/accel/accel.c` (4357줄, 마커 294) — submit_dif_generate까지. **잔여 = dif_generate_copy(~1695줄) 이후 ~2450줄**(accel sequence 상태머신, channel, key, opts) → batch6에서 이어감.
  - ◐ `lib/blob/blobstore.c` (11860줄, 마커 747) — blob_load_snapshot_cpl까지. **잔여 = blob_load_esnap(~2493줄) 이후**(persist 체인, 4077~5340 갭 등) → batch6에서 이어감.
  - ◐ `lib/nvmf/vfio_user.c` (7695줄, 마커 833) — destroy_endpoint까지. **잔여 = max_queue_size(~2256줄)~4500줄 hot-path 헬퍼** → batch6에서 이어감.
- ◐ batch5(진행 중) — `bdev_nvme.c`(이어), `bdev.c`(이어), `lib/nvmf/ctrlr_bdev.c`, `module/bdev/aio/bdev_aio.c`, `module/bdev/crypto/vbdev_crypto.c`, `lib/sock/sock.c`.
- ◐ batch6(진행 중) — `accel.c`(이어), `blobstore.c`(이어), `vfio_user.c`(이어), `module/bdev/malloc/bdev_malloc.c`, `module/bdev/null/bdev_null.c`, `lib/nvmf/transport.c`.
...
1. **NVMe 드라이버 다섯 기둥 완결 이후 확장 작업**:
   ...
   - ☑ `lib/nvme/nvme_ctrlr.c` — Part 9 완료(2026-05-28): 누적 113종 보강, 사실상 완결. 다음 세션은 별도 파일 선택 (lib/nvme/nvme_pcie* 또는 lib/nvme/nvme_qpair.c 권장).
...
- ☑ **lib/nvme/nvme_ctrlr.c** — 5997 → ... → **~10500**줄. v1 8종 + v2 qpair 10종 + v3 features/fail 9종 + v4 shutdown 9종 + v5 disconnect/reconnect 12종 + v6 reset/AER chain/process_init sub-callbacks 34종 + v7 namespace identify chain 7종 + multi-process 6종 + v8 boot/security/firmware 7종 + **v9 namespace mgmt 4종 + CMB/PMR 7종** = **누적 113종 보강 — 사실상 완료**. 잔여 trivial helper 만 남음 (get_flags/get_transport_id/alloc_qid 등 단순 getter, 별도 세션 필요시 일괄 처리).

  **Part 9 (2026-05-28 세션) 보강 내역 — namespace mgmt + CMB/PMR**:
  - **namespace management** (4종): spdk_nvme_ctrlr_attach_ns (opcode 0x15 SEL=0x00 + identify_active_ns + ns_construct), spdk_nvme_ctrlr_detach_ns (SEL=0x01), spdk_nvme_ctrlr_create_ns (opcode 0x0D SEL=0x00, cdw0 = 새 NSID 반환), spdk_nvme_ctrlr_delete_ns (SEL=0x01).
  - **CMB (Controller Memory Buffer)** (3종): spdk_nvme_ctrlr_reserve_cmb (CMBSZ.RDS/WDS 검증 + sz*(0x1000<<szu*4) 크기 계산), spdk_nvme_ctrlr_map_cmb (BAR mmap), spdk_nvme_ctrlr_unmap_cmb.
  - **PMR (Persistent Memory Region)** (4종): spdk_nvme_ctrlr_enable_pmr (PMRCTL.EN), spdk_nvme_ctrlr_disable_pmr, spdk_nvme_ctrlr_map_pmr, spdk_nvme_ctrlr_unmap_pmr.

  **★ 핵심 인사이트 (Part 9)**:
  - **NVMe ns 생명주기 = create/attach/detach/delete 분리**: ns 자체 생성 (create_ns, opcode 0x0D SEL=0x00) 과 컨트롤러 binding (attach_ns, opcode 0x15) 가 분리된 명령. multi-controller subsystem 에서 한 ns 를 여러 컨트롤러에 attach 가능. delete 는 영구 삭제, detach 는 binding 만 해제.
  - **create_ns 반환값 = cdw0**: 다른 함수는 errno (int) 반환이지만 create_ns 만 uint32_t (NSID). NVMe spec 이 cdw0 에 NSID 담아 반환하는 패턴 — 0=실패, >0=NSID. nvme_wait_for_adminq_completion(false) 로 command status 검사 위임하고 호출자가 cdw0 직접 사용.
  - **active_ns 재 수집 일관 패턴**: attach/detach/delete 모두 완료 후 nvme_ctrlr_identify_active_ns 동기 호출로 트리 갱신. ns 객체 자체는 attach_ns 가 추가로 nvme_ns_construct 까지 호출 (nsdata 채워야 IO 가능).
  - **CMB 크기 인코딩**: CMBSZ 의 sz * (0x1000 << szu*4) — szu (Size Unit) 가 0..5 로 4KB~4GB 의 단위, sz 가 그 단위의 개수. 4GB 초과는 표현 불가 (spec 한계).
  - **CMB vs PMR vs BAR**: 모두 컨트롤러 메모리지만 의도가 다름. CMB = IO 가속 (PRP/SQE/CQE/데이터 가능), PMR = 영속 메모리 (전원 차단 후 보존), 일반 BAR = doorbell/레지스터. CMB 는 RDS+WDS 둘 다 있어야 데이터 용 사용 가능.
  - **transport ops 위임 패턴**: 모든 CMB/PMR 함수가 ctrlr_lock 으로 감싼 transport_ctrlr_*_cmb/pmr 호출 thin wrapper. PCIe transport 가 실제 BAR mmap 처리, fabrics 는 ENOTSUP. 락은 다른 transport 조작 (reset 등) 과 race 차단.

  **Part 8 (2026-05-28 세션) 보강 내역 — boot/security/firmware API**:
  - **firmware update** (1종 — 가장 복잡): spdk_nvme_ctrlr_update_firmware (페이지 단위 download loop + commit + NVM_RESET vs CONVENTIONAL_RESET 분기 + spdk_nvme_ctrlr_reset 마무리).
  - **boot partition read** (2종): spdk_nvme_ctrlr_read_boot_partition_start (BPMBL physical addr write + BPRSEL trigger), spdk_nvme_ctrlr_read_boot_partition_poll (BRS 상태 4가지 분기).
  - **boot partition write** (2종 — 4-state machine): nvme_write_boot_partition_cb (DOWNLOADING → DOWNLOADED → REPLACE → ACTIVATE 콜백 체인), spdk_nvme_ctrlr_write_boot_partition (state 초기화 + 첫 download 발행).
  - **security** (2종): spdk_nvme_ctrlr_security_receive (opcode 0x82, device→host), spdk_nvme_ctrlr_security_send (opcode 0x81, host→device).
  - **format** (1종): spdk_nvme_ctrlr_format (Format NVM opcode 0x80 + format 후 reset 필수).

  **★ 핵심 인사이트 (Part 8)**:
  - **firmware update 의 2-phase + reset 처리**: download (페이지 단위 0x11) + commit (0x10) + NVMe spec 의 "commit 응답이 NVM_RESET 또는 CONVENTIONAL_RESET 요구" 분기. NVM_RESET 만 본 API 가 자동 reset, CONVENTIONAL_RESET 은 호스트 PCIe reset 책임이라 NOTICELOG 후 음수 반환.
  - **★ boot partition 4-state machine**: write_boot_partition 은 단일 사용자 호출 + 4단계 admin chain (download * N → commit(REPLACE) → commit(ACTIVATE)) 을 콜백이 자동 진행. ctrlr->bp_ws 가 상태 추적, fw_offset/fw_size_remaining/fw_payload/fw_transfer_size 가 진행 변수. 사용자 콜백은 최종 1회만 호출 (성공 시 ACTIVATE 끝, 실패 시 발생 시점).
  - **BPMBL physically contiguous 요구**: boot partition read 의 DMA 목적지는 BPMBL 단일 base address 만 받으므로 PRP list 같은 분산 불가 — spdk_vtophys 반환 크기 검사로 보장. NVMe spec §3.1.5.2 의 단순화 (boot 경로는 minimal hardware).
  - **format 후 reset 필수**: namespace identify 결과 (LBA size, metadata size, PI type) 가 무효화되므로 reset 으로 init state machine 재실행 → 새 ns 메타데이터 수집.
  - **security send/receive = SPC-4 + Opal SED 통로**: 별도 NVMe-specific 명령 없이 SPC-4 의 SECURITY PROTOCOL IN/OUT 매핑만으로 모든 보안 기능 (Opal challenge-response, TCG Storage) 처리. secp/spsp/nssf 3개 필드가 프로토콜·서브명령 구분.
  - **status tracker heap 할당 패턴 일관성**: format/firmware/security/boot 모두 nvme_completion_poll_status 를 calloc 으로 heap 할당. 이유: timeout 시 콜백이 늦게 도착해도 dangling 회피 (스택 변수면 함수 반환 후 corruption). nvme_wait_for_adminq_completion 이 timeout 시 status 보존 권한 (timed_out=true 면 호출자가 free 하지 않음).

  **Part 7 (2026-05-28 세션) 보강 내역 — namespace identify chain + multi-process**:
  - **namespace identify chain** (7종): nvme_ctrlr_identify_active_ns_async_done (CNS 0x02 페이징 콜백, 1024 NSID/page, sentinel=0), nvme_ctrlr_identify_active_ns_async (CNS 0x02 발행 또는 NVMe 1.0/quirk 시 가짜 목록 생성), _nvme_active_ns_ctx_deleter (DONE 시 swap·destroy·state IDENTIFY_NS 전이), _nvme_ctrlr_identify_active_ns (state machine 진입점 — deleter 등록), nvme_ctrlr_identify_active_ns (동기 wrapper — reset 경로), nvme_ctrlr_identify_ns_async_done (CNS 0x00 per-ns 순회 콜백 → 다음 ns), nvme_ctrlr_identify_ns_async (CNS 0x00 발행), nvme_ctrlr_identify_namespaces (chain 진입점 first ns), nvme_ctrlr_identify_namespaces_iocs_specific_next (CNS 0x05 ZNS / 0x08 NVM iterator).
  - **multi-process** (6종): nvme_ctrlr_get_process (PID → ctrlr_process), nvme_ctrlr_get_current_process (getpid wrapper), nvme_ctrlr_add_process (probe/attach 시 등록, SPDK_MALLOC_SHARE), nvme_ctrlr_remove_process (정상 detach, qpair free + pci_device_detach), nvme_ctrlr_cleanup_process (비정상 종료 강제 정리, in_completion_context=0 + no_deletion_notification), nvme_ctrlr_proc_get_ref / put_ref (ref counting + lazy remove_inactive_proc).

  **★ 핵심 인사이트 (Part 7)**:
  - **NVMe 1.1+ CNS 0x02 페이징 규약**: 한 페이지=1024 NSID, 마지막 슬롯이 0 sentinel 이면 "더 없음", 비제로면 다음 페이지의 starting NSID. 본 콜백이 page_count++ + spdk_realloc(page_size 정렬) + admin 재발행으로 페이징 루프 형성.
  - **NVMe 1.0/quirk fallback**: CNS 0x02 미지원 컨트롤러는 cdata.nn 으로부터 "1..nn 전체가 active" 가짜 목록을 즉시 채워 DONE 직행. 페이지 단위 올림 할당 + nn 위치에 0 sentinel.
  - **동기 vs 비동기 경로 차이**: nvme_ctrlr_identify_active_ns_async 가 deleter 유무로 분기 — deleter 있으면 async 콜백이 swap·destroy·state 전이 자동, NULL 이면 호출자가 nvme_wait_for_adminq_completion 후 직접 swap.
  - **★ active_procs fan-out + SPDK_MALLOC_SHARE**: primary/secondary 모두 같은 ctrlr_process 객체를 보기 위해 spdk_zmalloc(SPDK_MALLOC_SHARE) 로 hugepage 공유 풀에서 할당 — DPDK shared memory 통해 cross-process 가시성.
  - **★ remove vs cleanup 비대칭**: remove_process 는 active_reqs 비어 있음 assert (정상 detach), cleanup_process 는 강제 free (kill 등) + in_completion_context=0 강제 reset + no_deletion_notification_needed=1 (Delete IOSQ admin skip — 죽은 프로세스 위해 발행 무의미).
  - **lazy inactive cleanup**: get_ref/put_ref 가 매번 nvme_ctrlr_remove_inactive_proc 호출 (kill(pid,0)==-1 && ESRCH 로 죽은 프로세스 감지) — 별도 reaper thread 없이 다음 ref 조작이 청소 트리거.
  - **마지막 proc 예외**: put_ref 가 ref=0 도달 시 즉시 remove_process 하지만, proc_count==1 (마지막 프로세스) 이면 보존 — ctrlr destruct 가 일괄 처리 (nvme_ctrlr_free_processes).
...
- 2026-05-28 · **lib/ublk/ublk.c Part 5** — **ublk_start_disk** §2 멀티라인 + 인라인 (★ /dev/ublkbN 노출 진입점 — RPC ublk_start_disk → 사전 검증 (app_thread + active + ublk_id 중복 + max_dev) + ublk_dev calloc + bdev open + sector_per_block_shift 계산 + 비동기 chain 시작 ADD_DEV → SET_PARAMS → START_DEV). 중복 함수 잔재 정리 (이전 Edit 의 부작용 — old ublk_ctrl_cmd_submit 사본 제거). **ublk.c 진행도 ~45%**. **인사이트**: ublk_start_disk = 비동기 multi-stage chain 의 진입점 (ADD → SET_PARAMS → START), bdev event_cb 등록으로 hot-remove 자동 close, sector(512B) → block (bdev 단위) 변환 shift 정확성.

- 2026-05-28 · **lib/ublk/ublk.c Part 4** — control plane 2 함수 §2 멀티라인: (1) **ublk_ctrl_poller** (/dev/ublk-control CQE 폴링 — peek_cqe + cqe_seen 패턴 (batching 8, data plane 과 분리), in-progress=0 시 즉시 IDLE CPU 절약), (2) **ublk_ctrl_cmd_submit** (★ ublk lifecycle 명령 SQE 빌드 통합 — UBLK_CMD_* 8종 (ADD/GET_DEV_INFO/SET_PARAMS/START/STOP/DEL/START_USER_RECOVERY/END_USER_RECOVERY), io_uring_cmd payload 24B 인라인, owner pid 추적 (START_DEV/END_USER_RECOVERY), inflight 양쪽 카운트 (전체 + per-dev)). **ublk.c 진행도 ~40%**. 인사이트: control vs data plane 분리 (별도 io_uring, 별도 poller 빈도), peek+seen vs for_each_cqe (control 은 응답 분기 복잡), owner pid 추적으로 daemon crash auto-cleanup, in-flight 양쪽 카운트 (전체 poller IDLE 결정 + per-dev destroy 대기).

- 2026-05-28 · **lib/nvmf/vfio_user.c Part 9 + lib/ublk/ublk.c Part 3** — vfio_user.c 4 함수 §2 + ublk.c 2 함수 §2 보강. **vfio_user.c**: (1) **handle_create_io_q wrapper** (공통 검증 + SQ 성공 시 비동기 CQE post_create_io_sq_completion 분기 — 중복 CQE 방지), (2) **_free_ctrlr + free_ctrlr** (controller thread dispatch — vfu_ctx_poller/intr unregister 안전성, endpoint relisten vs async_destroy 분기), (3) **vfio_user_ctrlr_set_intr_mode** (endpoint 에 mode 저장 — controller reset 후 새 ctrlr 상속, shadow doorbell rearm), (4) **nvmf_vfio_user_listen** (★ endpoint 생성 — traddr 중복 검사, BAR0 backing file open+unlink+ftruncate+mmap, vfu_create_ctx SOCK + accept_poller 등록). **ublk.c**: (5) **ublk_create_target** (★ target 활성화 — singleton + cpumask 파싱 + ublk_open + per-core thread+poll_group + ctrl_poller), (6) **ublk_dev_queue_init** (★ per-queue setup — io_cmd_buf mmap (PROT_READ+MAP_POPULATE, 큐별 offset), io_uring SQE128 setup, fixed file register cdev_fd → SQE fd_index 사용 (fdget 비용 절감), init_io_cmds 불변 필드 미리 채움). **vfio_user.c 진행도 ~95%** / **ublk.c 진행도 ~30%**. 인사이트: vfio-user 의 endpoint 생성 시 BAR0 anonymous file 트릭 (open+unlink), io_uring SQE128 (ublk_drv 페이로드 수용), fixed file fd_index, post_create_io_sq_completion 비동기 CQE 분기, controller thread dispatch (vfu_ctx_poller unregister 안전성).

- 2026-05-28 · **lib/ublk/ublk.c Part 2** — io_uring 양방향 hot path 2 함수 §2 멀티라인 + 인라인: (1) **ublk_io_xmit** (★ UBLK_IO_COMMIT_AND_FETCH_REQ 통합 명령 패턴 — 응답 + 다음 fetch 묶기로 syscall 절반, scan-build use-after-free 회피 트릭 (미리 list remove), SQPOLL 미사용 가정 하 submit 직후 buffer free 정책), (2) **ublk_io_recv** (★ io_uring CQE dispatcher — user_copy 플래그로 2 경로 분기, COMMIT 응답 경로 (OK/NEED_GET_DATA/ABORT/error), user_copy 완료 경로 (READ free 직접 / WRITE bdev 발행), UBLK_QUEUE_REQUEST=32 batching 한계 (다른 queue starvation 회피), io_uring_cq_advance ring head 갱신). **인사이트**: COMMIT_AND_FETCH 통합 명령으로 NBD 대비 syscall ½, user_copy vs zero-copy 2-pass 패턴, ABORT 응답 자동 stopping 전이, batching 한계로 queue fairness, scan-build 트릭 (pre-remove pattern).

- 2026-05-28 · **lib/ublk/ublk.c Part 1** (초기 보강 시작) — 2103줄 미착수 파일에서 상단 4섹션 블록은 이미 완비 상태였고 (sessions 이전에 작업됨), 본 라운드에서 3 hot path 함수 §2 멀티라인 + 인라인 추가: (1) **ublk_poll** (★ ublk transport hot path — TAILQ_FOREACH_SAFE 로 모든 queue 순회, xmit 우선 (응답 → 사용자 syscall unblock latency 감소) → recv (커널 CQE → bdev 발행), is_stopping queue 자동 close), (2) **ublk_io_done** (bdev I/O 완료 콜백 — success 시 io->result 보존 / 실패 시 -EIO, inflight → completed list 이동 — 다음 ublk_poll iter 가 xmit 처리), (3) **_ublk_submit_bdev_io** (★ ublk_drv IO opcode → SPDK bdev API 5종 1:1 매핑 — READ/WRITE/FLUSH/DISCARD/WRITE_ZEROES, sector_per_block_shift 단위 변환, user_copy 모드 분기, -ENOMEM 시 wait queue). 잔여: ublk_io_recv (io_uring CQE 파싱), ublk_io_xmit (commit-and-fetch SQE), ublk_dev_queue_init/fini, ublk_create_target, spdk_ublk_start_disk, ublk_start_dev, recovery 경로. **인사이트**: xmit 우선 호출 (응답 latency 최소화), polled-mode io_uring, sector(512B) → block 변환, user_copy 모드 vs zero-copy, -ENOMEM resubmit pattern.

- 2026-05-28 · **lib/nvmf/vfio_user.c Part 8** — consume_admin_cmd dispatcher 보강 (★ admin opcode 분기 — Create/Delete IO Q + Doorbell Buffer Config 는 vfio-user 가 직접 처리, 나머지는 공통 ctrlr layer 위임). Fused admin 미지원 처리. **vfio_user.c 5076줄 진행도 ~85%**. 잔여: handle_create_io_q wrapper, free_ctrlr, set_ctrlr_intr_mode, listen 등.

- 2026-05-28 · **lib/nvmf/vfio_user.c Part 6+7** — 핵심 7 함수 §2 멀티라인 + 인라인: (1) **handle_del_io_q** (★ SQ vs CQ delete 비대칭 — CQ 즉시 (cq_ref=0 검증), SQ 비동기 (delete_ctx 동적 할당 + qpair_disconnect → drain → delete_sq_done 콜백 CQE post)), (2) **vfio_user_property_access** (★ BAR0 NVMe register read/write → in-memory Fabric Property Get/Set capsule build → spdk_nvmf_request_exec 으로 공통 ctrlr layer 라우팅 — RDMA/TCP/vfio-user 가 같은 ctrlr 코드 공유 핵심 트릭, req->cc 보존으로 prop_set_cc 전후 비교), (3) **nvmf_vfio_user_accept** (per-endpoint accept poller — 1 endpoint = 1 controller 한도, vfu_attach_ctx 성공 시 ★ 자가 unregister (이후 vfu_ctx_poller 가 처리)), (4) **map_admin_cmd_req** (admin opcode 별 데이터 길이 — IDENTIFY=4KB 고정, GET_LOG_PAGE 32-bit numdw, FW_DOWNLOAD numd, SET_FEATURES fid 별), (5) **map_io_cmd_req** (IO 단순 — get_nvmf_io_req_length + vfio_user_map_cmd PRP→iov), (6) **memory_region_add/remove_cb** (★ VM DMA register/unregister 이벤트 — 2MB 정렬 강제 (SPDK hugepage 단위), INACTIVE↔ACTIVE SQ/CQ remap, shadow doorbell 영역 제거 시 fallback + free_sdbl), (7) **vfio_user_poll_vfu_ctx** (★ vfu_run_ctx 단일 작업 — 모든 vfio-user 이벤트 처리 (BAR/DMA/IRQ/message), ENOTCONN = VM 종료 처리), (8) **handle_doorbell_buffer_config** (★ NVMe 1.3 Doorbell Buffer Config opcode 0x7C — PRP1/PRP2 두 page GPA → host VA 매핑, dstrd × MAX_QPAIRS ≤ page_size 검증, CQ head doorbell = NEVER_NOTIFY polling 고정, SWAP 트릭으로 safe activate, live migration 용 PRP 보존), (9) **nvmf_vfio_user_create_ctrlr** (★ VM connect 후 controller 생성 — cntlid generation (RDMA/TCP/vfio-user 공존 시 충돌 회피), admin SQ/CQ 미리 준비 (AQ depth=32), spdk_nvmf_tgt_new_qpair → poll_group_add). 잔여: handle_create_io_q (sq/cq wrapper), consume_admin_cmd, nvmf_vfio_user_listen, free_ctrlr, set_ctrlr_intr_mode 등 보조 함수. **인사이트**: Fabric Property capsule 라우팅 (3 transport 공통 ctrlr layer 활용), accept poller 자가 unregister (1:1 endpoint 한도 + CPU 절약), 2MB 정렬 강제 (hugepage), shadow doorbell PRP live migration 보존, cntlid generation 의 multi-transport 공존 고려, SQ/CQ delete 비대칭 (CQ 즉시 / SQ 비동기), VM 종료 ENOTCONN fast path.

- 2026-05-28 · **lib/nvmf/vfio_user.c Part 5** — handle_create_io_cq §2 멀티라인 헤더 + 인라인. CQ-vs-SQ 차이 인사이트 정리 (iv = MSI-X vector index, ien = IRQ trigger 활성, phase=true 초기화, cq_state=CREATED 이후 SQ binding 가능). 잔여 vfio_user.c 보조 함수 (delete_io_sq/cq, property_access, create_ctrlr, map_admin/io_cmd_req, accept_poller, Shadow Doorbell Buffer Config) 는 다음 세션. **인사이트**: NVMe Create I/O Q 의 SQ/CQ 비대칭 (IV/IEN 은 CQ 만, qprio/cqid 는 SQ 만), MSI-X vector 한도 검증 (NVMF_VFIO_USER_MSIX_NUM-1), phase=true 초기화로 첫 wrap 전 새 CQE 식별.

- 2026-05-28 · **lib/nvmf/vfio_user.c Part 4** — Queue mgmt + property access + SQE fetch 3 핵심 함수 §2 멀티라인 보강: (1) **handle_create_io_sq** (★ Create I/O SQ admin opcode 0x01 — cdw10/11 비트필드 디코드 (qid/qsize+1/cqid/pc/qprio), CQ 선행 생성 검증 (spec § 5.5.1 강제 순서), PC=1 only (분산 큐 미지원), map_q 로 VM SQ memory → host VA 매핑, alloc_sq_reqs 로 풀 calloc, cq_ref++, shadow doorbell race 처리 set_sq_eventidx), (2) **handle_dbl_access** (★ BAR0 0x1000+ doorbell write 처리 slow path — mappable BAR 미사용 시만 호출, write-only spec 검증, dword aligned, byte → array index 변환 (짝수=SQ tail, 홀수=CQ head), DSTRD=0 NVMf 고정), (3) **handle_sq_tdbl_write** (★ SQE fetch loop hot path — flow control (free_cq_slots vs nr_outstanding) for VM Linux driver 과민 enqueue 방어, sq_head_advance 전 위치 (CQE 의 SQHD 정확성), shadow doorbell need_rearm IO SQ 한정, cq_full + need_kick + cq_full 통계). **잔여 (다음 세션 Part 5)**: handle_create_io_cq, handle_delete_io_sq/cq, vfio_user_property_access (CC.EN/SHN write 처리 — controller enable/disable/shutdown), nvmf_vfio_user_create_ctrlr, map_admin_cmd_req / map_io_cmd_req (PRP→iov), accept_poller (vfu_accept loop), Shadow Doorbell Buffer Config (0x7F Set Features 처리), AER/IO/admin path 보조 함수. 인사이트: NVMe Create IO SQ 의 spec 강제 순서 (CQ 선행), PC=1 only (분산 큐는 PRP list 형식 — vfio-user 미구현), VM 메모리 직접 read (zero-copy SQE), CQ flow control on SQ side (NVMe spec 약점 보강), sq_head_advance 전 위치 (CQE SQHD 일관성), DSTRD=0 NVMf 고정 정책, shadow doorbell race window (set_sq_eventidx 전 VM doorbell write = fail_ctrlr).

- 2026-05-28 · **lib/nvmf/vfio_user.c Part 3** — IO path 핵심 4 함수 §2 멀티라인 + vtable 완비: (1) **handle_cmd_req** (★ SQE → nvmf_request 변환 + admin/io 분기 — get_nvmf_vfio_user_req 풀 빌림, SQE 사본 (VM 메모리 disturbance 회피), Reservation/Fabric 미지원 -ENOTSUP, map 실패 시 즉시 CQE 발행 + 풀 반환), (2) **post_completion** (★ CQE 16B VM CQ 메모리 write + IRQ trigger — CQ full=버그 (SQ 측 flow control), bitfield local 변수 조립으로 RMW 회피, wmb + cq_tail_advance 순서, adaptive_irqs 조건 분기), (3) **nvmf_vfio_user_sq_poll** (doorbell 검사 → handle_sq_tdbl_write → handle_cmd_req 체인 + suppressed IRQ 강제 발행), (4) **spdk_nvmf_transport_vfio_user vtable** (★ 18+ callback 그룹별 인라인 + "muser" historical 이름 + IO 흐름 설명). **잔여 (다음 세션 Part 4)**: handle_create_io_sq/cq (Create IO Q admin 처리 + DMA mapping), handle_delete_io_sq/cq, vfio_user_property_access (CC write 시 enable/disable/shutdown 처리), handle_sq_tdbl_write (SQE fetch loop), map_admin_cmd_req / map_io_cmd_req (PRP→iov), nvmf_vfio_user_create_ctrlr (controller 생성), pci config 셋업, shadow doorbell sb_cmp_and_set, accept_poller. 인사이트: SQE 사본으로 VM 메모리 disturbance 회피, CQE bitfield RMW 회피 (PCIe 왕복 절약), IRQ adaptive_irqs (admin = always, IO = lazy with handle_suppressed_irq fallback), Reservation/Fabric 미지원 (vfio-user 의 한계), "muser" historical naming.

- 2026-05-28 · **lib/nvmf/vfio_user.c Part 2** — ctrlr/endpoint/transport 3 핵심 구조체 + transport_opts + 3 핵심 함수 §2 멀티라인 헤더: (1) **nvmf_vfio_user_ctrlr** (★ 1 VM = 1 ctrlr — sqs[512]/cqs[512] 배열 + bar0_doorbells + sdbl shadow doorbell + cntlid + 5-state quiesce machine + adaptive_irqs), (2) **nvmf_vfio_user_endpoint** (★ 1 endpoint = 1 socket file = 1 VM — vfu_ctx + accept_poller (별도 스레드) + bar0 mmap + pci_config_space + msix), (3) **transport_opts** (5 옵션: disable_mappable_bar0/adaptive_irq/shadow_doorbells/compare + enable_intr_mode_sq_spreading), (4) **nvmf_vfio_user_transport** (전체 transport — endpoints + poll_groups + RR next_pg cursor + intr_mode_supported capability), (5) **access_bar0_fn** (★ VM BAR0 read/write trap → 0x1000+ doorbell 분기 vs 0x0~0x0FFF NVMe register 분기, mappable BAR mmap 우회 효과 설명), (6) **vfio_user_ctrlr_intr** (★ interrupt mode 핸들러 — vfu_ctx poll 우선 (doorbell 처리) → SQ poll, intr_mode_sq_spreading 시 cross-reactor wake-up eventfd_write, sqs[0] NULL safety check), (7) **nvmf_vfio_user_poll_group_poll** (★ hot path — vfu_ctx_poller 와 분리 책임 (vfu_ctx 무거움/SQ 가벼움), ACTIVE+size>0 SQ 만 polling, _SAFE iteration). **잔여 (다음 세션 Part 3)**: handle_cmd_req (admin SQE 처리), handle_create/delete_io_sq/cq, vfio_user_property_access (CC write 처리), nvmf_vfio_user_sq_poll, post_completion (CQE write + IRQ trigger), shadow doorbell sb_cmp_and_set, vtable. 인사이트: BAR0 layout (0x00 NVMe regs / 0x1000+ doorbells), mappable BAR 의 trap 회피로 성능 (shadow doorbell + mmap), vfu_ctx_poller 와 SQ poller 분리 (책임 + 빈도 분리), interrupt mode 의 cross-reactor SQ spreading 처리, 5-state quiesce 머신 (live migration 안전 처리).

- 2026-05-28 · **lib/nvmf/vfio_user.c Part 1** (초기 보강 시작) — 5076줄 미착수 파일에 (a) 상단 4섹션 블록 (NVMe-oF vfio-user transport 아키텍처 — VM Guest → vfio-pci → vfio-user Unix socket → libvfio-user → 본 transport → bdev, RDMA/TCP 와의 본질적 차이, shadow doorbell + eventidx 패턴, 핵심 상수 BAR 크기 산식) (b) request 2-state enum (RDMA 의 16-state 와 대비) (c) 핵심 7 구조체 §2 멀티라인 헤더: nvmf_vfio_user_req (가변 길이 sg[] 패턴), nvme_q_mapping (VM SQ/CQ 메모리 host 측 mapping), sq_state 5종 enum, cq_state 3종 enum, ctrlr_state 5종 enum (CREATING → RUNNING ↔ PAUSING → PAUSED → RESUMING 의 quiesce 흐름), nvmf_vfio_user_sq (qpair + dbl_tailp + need_rearm + cqid), nvmf_vfio_user_cq (tail + dbl_headp + phase + iv + ien + nr_outstanding), nvmf_vfio_user_shadow_doorbells (NVMe 1.3 spec § 4.10 — shadow_doorbells + eventidxs 의 polling/notify 전환 패턴). **잔여 (다음 세션 Part 2)**: nvmf_vfio_user_ctrlr, nvmf_vfio_user_endpoint, nvmf_vfio_user_transport 구조체 + 핵심 함수 (handle_bar_handler, handle_cc/cq/sq, vfio_user_ctrlr_intr, request lifecycle, vtable). 인사이트: vfio-user 는 host 안의 VM 에 가상 NVMe PCIe device 노출 (네트워크 없음), shadow doorbell 로 VM exit 최소화, quiesce → pause → resume 의 live migration 지원, request 의 가변 길이 sg[] 트릭 (dma_sg_t 크기가 libvfio-user 버전 의존이라 컴파일 타임 미정).

- 2026-05-28 · **lib/nvmf/rdma.c Part 5** — qpair lifecycle 마무리 + vtable 완비. 3 함수 + vtable 보강: (1) **nvmf_rdma_qpair_initialize** (★ 실제 ibverbs QP create + resources 풀 + STAILQ 초기화 — qp_context 캐시, pd/cq/srq 묶기, max_send_wr=depth*2, CQ resize, NIC 가 실제 부여한 값으로 max_* 재확정), (2) **nvmf_rdma_disconnect** (DISCONNECTED/DEVICE_REMOVAL+qp 처리 — ack 우선 후 qpair_disconnect chain), (3) **spdk_nvmf_transport_rdma vtable** (★ 21 callback entry 전체 인라인 주석 — identification/lifecycle/listener/poll_group/request/qpair/stats 그룹별 + 호출 흐름 read I/O 5단계 예시 + SPDK_NVMF_TRANSPORT_REGISTER constructor 패턴). **★ lib/nvmf/rdma.c 5403줄 사실상 완결** — Part 1~5 누적 (구조체 + state dispatcher + qpair lifecycle + resources + accept + CM event + SGL parsing + connect 협상 + QP create + vtable) 모든 핵심 경로 §2 멀티라인 + 인라인 완비. **잔여 (다음 세션 선택 사항)**: setup_request (RDMA WR remote addr/rkey 세팅), fill_wr_sgl(_with_dif), fused command 처리, dump_opts/cdata_init/listen/discover, opts validate, poll_group_dump_stat 등 보조 함수. **다음 권장 파일**: lib/nvmf/vfio_user.c (5076줄, 0건) 또는 lib/nvmf/fc.c (4013줄). 인사이트: QP init attr 의 qp_context = container_of 역추적 키, CQ resize 가 새 QP 추가 시 required_num_wr 누적 → 깊이 확장, NIC HW 가 부여한 값으로 재확정 (요청 < 실제 가능), vtable 21 callback 의 그룹별 분류 (lifecycle/listener/poll_group/request/qpair/stats), SPDK_NVMF_TRANSPORT_REGISTER 의 ELF constructor 자동 등록, host read I/O 5단계 (capsule SEND → RECV CQE → parse_sgl → bdev exec → RDMA WRITE + SEND).

- 2026-05-28 · **lib/nvmf/rdma.c Part 4** — CM event 처리 + SGL parsing + connect 핵심 4 함수 §2 멀티라인 헤더: (1) **nvmf_process_cm_events** (★ 12+ CM event type 디스패치 — ADDR/ROUTE_RESOLVED skip, CONNECT_REQUEST → connect, DISCONNECTED → disconnect, DEVICE_REMOVAL 분기 처리, batching 32 + rdma_ack_cm_event 규약), (2) **nvmf_rdma_request_parse_sgl** (★ SGL 3종 디스패치 — KEYED_DATA_BLOCK + ADDR/INVALIDATE_KEY 가 hot path, LAST_SEGMENT 가 multi-SGL, DATA_BLOCK + OFFSET 은 ICD 잘못된 호출), (3) **nvmf_rdma_request_fill_iovs** (★ iobuf → ibv_sge 변환 + lkey 등록 + DIF 처리 (block_size 단위 split, num_wrs 계산), err_exit 자원 일괄 반환), (4) **nvmf_rdma_connect** (★ CONNECT_REQUEST 의 queue depth 4-min 협상 — target opts · local NIC max_qp_wr · remote initiator_depth · host hrqsize/hsqsize, RECFMT/responder_resources spec 검증, NUMA-aware QP 할당). **잔여 (다음 세션 Part 5)**: nvmf_rdma_qpair_initialize (실제 ibverbs QP create + rdma_accept), nvmf_rdma_disconnect, nvmf_rdma_setup_request (RDMA WR remote addr/rkey 세팅), nvmf_rdma_fill_wr_sgl(_with_dif), nvmf_rdma_request_get/free, transport_ops vtable (전체 callback table). 인사이트: SEND_WITH_INV 최적화 (host invalidate roundtrip 1회 절약), DIF 의 stripped_buffers 임시 영역 패턴 (PI strip 후 re-insert), queue depth 4-min 협상 (4개 한도의 가장 작은 값), responder_resources 0 강제 (host RDMA READ 안 함), CM event lifecycle (CONNECT_REQUEST → ESTABLISHED → DISCONNECTED → TIMEWAIT_EXIT).

- 2026-05-28 · **lib/nvmf/rdma.c Part 3** — switch case 본체 (RECV/RDMA_READ/WRITE) 인라인 보강 + 추가 6 함수 §2 멀티라인 헤더: (1) **nvmf_rdma_resources_create** (★ 5종 풀 일괄 할당 + RECV pre-post 패턴 — RNR 회피 위해 max_queue_depth 만큼 미리 게시 필수), (2) **nvmf_rdma_qpair_destroy** (★ 11단계 정리 순서 — in-flight 강제 정리 → poller 제거 → SRQ RECV 재게시 → ibverbs QP destroy → required_num_wr 감소 → resources destroy → cascading poller destroy → cm_id 마지막), (3) **nvmf_rdma_close_qpair** (graceful shutdown 진입 — to_close 마킹 + rdma_disconnect + 비동기 drain 시작), (4) **nvmf_rdma_destroy_drained_qpair** (★ drain 조건 4종: to_close+send_depth=0+recv_depth=max(non-SRQ)/last_wqe(SRQ)+fused_first=NULL), (5) **nvmf_rdma_accept** (transport-wide accept poller — RDMA CM 이벤트 + IB async 이벤트 통합 poll, non-blocking timeout=0), (6) **nvmf_rdma_qpair_process_pending** (★ 5단계 우선순위 PENDING 큐 처리 — SEND>READ>WRITE>iobuf>NEW pairing + fair processing TRANSFERRING skip). **잔여 (다음 세션 Part 4)**: nvmf_process_cm_events (CONNECT_REQUEST → accept → QP create), nvmf_rdma_qpair_create (QP setup), nvmf_rdma_request_fill_iovs (SGL→iov 변환), nvmf_rdma_request_parse_sgl, fused command 처리, transport ops table (vtable callbacks). 인사이트: PENDING 큐 우선순위 (read 제약 > write), 5종 풀 (reqs/recvs/cmds/cpls/bufs) hugepage 할당, RNR 회피용 RECV pre-post, drain 조건의 4종 (state coherency 보장), close→drain→destroy 비동기 chain, transport-wide accept_poller 가 모든 device 의 async event 까지 통합 처리.

- 2026-05-28 · **lib/nvmf/rdma.c Part 2** — Part 1 의 구조체에 이어 hot path 핵심 함수 3종 §2 멀티라인 헤더 + 인라인 보강: (1) **nvmf_rdma_request_process** (★ 16-state machine dispatcher — do-while progress loop + 에러 fast path 의 STAILQ_REMOVE 정확성 + back-to-back 전이 패턴 설명), (2) **nvmf_rdma_poll_group_poll** (poll_group → poller 순회 + fault isolation 패턴 + TAILQ_FOREACH_SAFE 이유), (3) **nvmf_rdma_poller_poll** (★ ibv_poll_cq batching 32 + wr_id type tag dispatch + SEND/RECV/RDMA_READ/WRITE 분기 + need_destroy fast path + polled-mode + idle_polls 통계 의미). **잔여 (다음 세션 Part 3)**: poller_poll 의 switch case 본체 (RECV/RDMA_READ/RDMA_WRITE 처리), qpair lifecycle (create/destroy/close/disconnect), RDMA CM 이벤트 핸들러 (rdma_get_cm_event + accept/reject/disconnect), accept_poller, resources_create/destroy, fused command 처리, in-capsule data 처리, transport opts 디코더. 인사이트: hot path 의 lockless 보장 (qpair affinity), wr_id type tag 패턴 (rdma_wr->type 으로 SEND/RECV/RDMA_READ/WRITE 분기), batching 32 의 syscall amortization, current_send/read_depth 감소 = num_outstanding_data_wr+1 (RDMA WRITE chain + SEND).

- 2026-05-28 · **lib/nvmf/rdma.c Part 1** (초기 보강 시작) — 5403줄 미착수 파일에 (a) 상단 4섹션 블록 (NVMe-oF RDMA target 아키텍처 + 16-state machine 다이어그램) (b) request 16-state enum 멀티라인 (FREE→NEW→NEED_BUFFER→...→COMPLETED 각 의미 + PENDING 상태가 RDMA depth 부족 대기임을 명시) (c) 기본 매크로 (NVMF_DEFAULT_MSDBD/TX/RX_SGE, MAX_WR_PER_QP, CQ_SIZE) 산정 근거 (d) 핵심 5개 구조체 (rdma_recv/rdma_request_data/rdma_request/rdma_qpair/rdma_poller) §2 멀티라인 헤더 + 필드별 보강. **잔여 (다음 세션)**: spdk_nvmf_rdma_transport / device / port / poll_group / conn_sched 구조체 + 함수 70여 개 (nvmf_rdma_request_process state dispatcher / nvmf_rdma_poll_group_poll / nvmf_rdma_qpair_destroy / RDMA CM 이벤트 핸들러 등). 인사이트: 16-state machine + PENDING 패턴 (queue depth 부족 시 대기 위치), SRQ 모드 (max_srq_depth>0 시 RECV 풀 공유로 메모리 절약), poller = (device, poll_group) 페어당 1개 + 같은 HCA QP 들이 단일 CQ 공유, RB tree (qp_num) 로 CQE → QP 역추적 O(log n), MAX_WR_PER_QP(depth) = depth*3+2 (RECV+SEND+RDMA READ/WRITE 합산).

- 2026-05-28 · **lib/nvme/nvme_internal.h zone_append API** (소규모 보강) — nvme_ns_cmd_zone_append_with_md / appendv_with_md 2종에 멀티라인 §2 헤더 추가. ZNS Zone Append (opcode 0x7D, TP 4053) 의 ALBA cdw0 반환 의미 + appendv vs append 분기 이유 (SGL 지원 컨트롤러 요구) 설명. 파일 전체는 다음 세션 추가 보강 대상 (fabric/rdma/tcp 일부 한 줄 주석을 §2 멀티라인으로 균일화 필요).

- 2026-05-28 · **lib/nvme/nvme_ctrlr.c Part 9** — namespace mgmt 4종 + CMB/PMR 7종 = 11종 보강. 누적 102→113종 = **nvme_ctrlr.c 사실상 완결**. 인사이트: ns 생명주기 = create/attach/detach/delete 분리 (multi-controller subsystem 지원), create_ns 반환값 = cdw0 NSID (uint32_t 특이 시그니처), active_ns 재 수집 일관 패턴, CMB 크기 인코딩 sz*(0x1000<<szu*4), CMB vs PMR vs BAR 의도 분리 (IO 가속 vs 영속 메모리 vs 레지스터), transport ops 위임 thin wrapper + ctrlr_lock.

- 2026-05-28 · **lib/nvme/nvme_ctrlr.c Part 8** — boot/security/firmware 7종 보강. 누적 95→102종. 잔여 ~4 (CMB/PMR/NS mgmt). 인사이트: firmware update 의 NVM_RESET vs CONVENTIONAL_RESET 분기 (NVM 만 자동 reset), boot_partition write 의 4-state machine (DOWNLOADING → DOWNLOADED → REPLACE → ACTIVATE 콜백 체인, 사용자 cb 최종 1회), BPMBL 의 physically contiguous 요구 (spdk_vtophys 반환 크기 검사), format 후 reset 필수 (ns 메타데이터 무효화), security send/receive = SPC-4 + Opal SED 통로, status tracker heap 할당 패턴 일관성 (timeout dangling 회피).

- 2026-05-28 · **lib/nvme/nvme_ctrlr.c Part 7** — namespace identify chain 7종 + multi-process 6종 = 13종 보강. 누적 82→95종. 잔여 ~11 (boot/security/firmware). 인사이트: NVMe 1.1+ CNS 0x02 페이징 (1024/page, sentinel=0), 1.0/quirk fallback (cdata.nn 으로 가짜 active 목록), SPDK_MALLOC_SHARE 로 active_procs cross-process 가시성, remove vs cleanup 비대칭 (정상 detach assert vs 강제 free + Delete IOSQ admin skip), lazy inactive cleanup (get_ref/put_ref 호출마다 kill(pid,0)==ESRCH 검사로 reaper thread 없이 청소), 마지막 proc 보존 (ctrlr destruct 가 일괄 처리).
