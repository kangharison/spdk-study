 ...
 * 호스트로 노출하여 chunk(=erase block) 단위 쓰기/리셋, 병렬 PU 활용,
 * 마모도 정보 조회, chunk 상태 통지 등을 지원한다. 본 헤더는 NVMe 기본
 * spec(nvme_spec.h) 위에 OCSSD vendor-specific opcode/log page/feature/구조체를
 * 얹어서, lib/nvme/nvme_ctrlr_ocssd_cmd.c, lib/nvme/nvme_ns_ocssd_cmd.c가
 * SQE(Submission Queue Entry)를 빌드할 때 정확한 바이트 레이아웃을 사용할 수
 * 있도록 한다. 즉 이 파일이 정의하는 비트필드 레이아웃은 그대로 PCIe 트랜잭션의
 * SQE 페이로드, Identify Geometry 응답 4096바이트, Chunk Information Log Page
 * 엔트리, Chunk Notification Log 엔트리에 매핑된다.
 *
 * === 전체 아키텍처에서의 위치 ===
 ...
 * 1.x 위에 정의된 OCSSD 1.2/2.0 와이어 포맷에 의해 보장된다.
 *
 * === 타 모듈과의 연결 ===
 ...
 *
 * === 주요 함수/구조체 요약 ===
 * - struct spdk_ocssd_dev_lba_fmt: 디바이스의 LBA 비트 분할(grp/pu/chk/lbk
 *   비트 길이) 기술. PPA를 평면 LBA로 인코딩할 때의 좌표 폭 정의.
 * - struct spdk_ocssd_geometry_data (4096B): Identify Geometry(opcode 0xE2)
 *   응답 페이로드. 버전, mccap(vector copy/multi-reset), num_grp/num_pu/
 *   num_chk/clba(기하), ws_min/ws_opt/mw_cunits/maxoc/maxocpu(쓰기 제약),
 *   tRD/tWR/tCRS 타이밍, vendor specific 영역으로 구성.
 ...
 * 길이를 64로 제한한다. lib/nvme/nvme_ns_ocssd_cmd.c가 이 상수를 가지고
 * lba_list 배열의 최대 크기를 검증하는 데 사용 — 호스트 FTL이 오버플로
 * 명령을 발급하지 못하도록 보호한다. */
...
struct spdk_ocssd_dev_lba_fmt {
	 ...
	 * 설정자: 디바이스. 읽는 자: 호스트 FTL(LBA→PPA 디코딩 시 시프트량). */
	 ...
	 * 값 범위: num_pu ≤ 2^pu_len. 설정자: 디바이스. 읽는 자: 호스트 FTL. */
	 ...
	 * 값 범위: num_chk ≤ 2^chk_len. 설정자: 디바이스. 읽는 자: 호스트
	 * FTL이 chunk 단위 reset/write 명령에서 PPA를 디코드할 때. */
	 ...
	 * 값 범위: clba ≤ 2^lbk_len. 설정자: 디바이스. 읽는 자: 호스트 FTL이
	 * write pointer 비교, sequential 검증 시 사용. */

	uint8_t reserved[4];
	/* [한국어] OCSSD spec 예약 영역(byte 12~15). 디바이스는 0으로 채워야
	 * 하며 호스트는 무시한다. 향후 spec 확장을 위한 placeholder.
	 * 설정자/읽는 자: 양쪽 모두 의미 부여 금지. */
};
...
struct spdk_ocssd_geometry_data {
	 ...
	 * 값 범위: 1(=1.2 호환), 2(=2.0). 설정자: 디바이스. 읽는 자: 호스트가
	 * 1.2/2.0 분기를 결정(필드 의미·opcode가 spec별로 다름). */
	...
	uint8_t		reserved1[6];
	/* [한국어] 예약(byte 2~7). 디바이스 0 채움, 호스트 무시. spec 확장 여지. */
	...
	struct {
		 ...
		 * 설정자: 디바이스. 읽는 자: 호스트 FTL이 device-side copy로
		 ...
		 * 설정자: 디바이스. 읽는 자: 호스트 FTL의 chunk 재사용 정책. */

		uint32_t	reserved	: 30;
		/* [한국어] mccap의 나머지 30비트 예약. 디바이스 0, 호스트 무시. */
	} mccap;

	uint8_t		reserved2[12];
	/* [한국어] 예약 영역(byte 20~31). 0 채움. */
	 ...
	 * 설정자: 디바이스. 읽는 자: 호스트 FTL이 chunk allocation pool 크기,
	 * GC victim 선택 시 후보 집합 크기 산출에 사용. */

	/** Chunk Size */
	uint32_t	clba;
	/* [한국어] chunk당 logical block 개수(섹터 단위).
	 * 와이어 위치: byte 72~75. 값 범위: 1~2^lbaf.lbk_len.
	 * 설정자: 디바이스. 읽는 자: 호스트 FTL이 chunk_information.wp가
	 * clba에 도달했는지(=chunk full) 검사할 때, 그리고 chunk 단위 sequential
	 * write 검증에 사용. */

	uint8_t		reserved4[52];
	/* [한국어] 예약(byte 76~127). spec 확장 영역. */
	 ...
	 * 설정자: 디바이스. 읽는 자: 호스트 FTL이 write 분할 시 ws_min 배수로
	 * 정렬 — 위반 시 SPDK_OCSSD_SC_OUT_OF_ORDER_WRITE(0xF2) 가능. */

	/** Optimal Write Size */
	uint32_t	ws_opt;
	/* [한국어] Optimal Write Size — 처리량 최적의 write 단위 sector 수.
	 * 와이어 위치: byte 132~135.
	 * 값 범위: ws_min의 배수 권장. 설정자: 디바이스.
	 * 읽는 자: 호스트 FTL이 streaming write의 청크 크기 선택. */
	 ...
	 * 와이어 위치: byte 136~139. 설정자: 디바이스. 읽는 자: 호스트 FTL의
	 * write-then-read 정합성 보장 정책. */

	/** Maximum Open Chunks */
	uint32_t	maxoc;
	/* [한국어] 디바이스 전체에서 동시에 open 상태로 둘 수 있는 chunk 최대 수.
	 * 와이어 위치: byte 140~143.
	 * 의미: maxoc를 초과해 open 시도 → 명령 실패. 설정자: 디바이스.
	 * 읽는 자: 호스트 FTL이 open chunk pool을 maxoc로 캡. */
	 ...
	 * 설정자: 디바이스. 읽는 자: 호스트 FTL이 PU별 open 카운터를 추적. */

	uint8_t		reserved5[44];
	/* [한국어] 예약(byte 148~191). spec 확장 placeholder. */

	/** tRD Typical */
	uint32_t	trdt;
	/* [한국어] read 명령의 평균 소요 시간(ns 단위가 일반적, vendor 정의).
	 * 와이어 위치: byte 192~195.
	 * 설정자: 디바이스. 읽는 자: 호스트 FTL의 latency QoS 추정/스케줄링. */
	 ...
	 * 와이어 위치: byte 200~203. 호스트 FTL의 write 큐잉 백프레셔 산정. */

	/** tWR Max */
	uint32_t	twrm;
	/* [한국어] write 명령의 최악 소요 시간.
	 * 와이어 위치: byte 204~207. timeout 산정에 사용. */

	/** tCRS Typical */
	uint32_t	tcrst;
	/* [한국어] chunk reset(=erase) 명령의 평균 소요 시간.
	 * 와이어 위치: byte 208~211. NAND erase는 보통 수 ms 단위.
	 * 호스트 FTL의 GC 스케줄링 비용 추정에 사용. */

	/** tCRS Max */
	uint32_t	tcrsm;
	/* [한국어] chunk reset 최악 시간.
	 * 와이어 위치: byte 212~215. 디바이스 측 reset timeout 산정. */

	/** bytes 216-255: reserved for performance related metrics */
	uint8_t		reserved6[40];
	/* [한국어] 예약(byte 216~255). 향후 성능 관련 지표(예: temperature
	 * derating coefficient 등)를 위한 placeholder. */

	uint8_t		reserved7[3071 - 255];
	/* [한국어] byte 256~3071 영역. 광범위 예약. spec 1.2/2.0에서 미사용 영역
	 * 이며 디바이스는 0으로 채운다. 호스트 파서는 이 영역을 건드리지 않는다.
	 ...
	 * 호스트 FTL은 일반적으로 무시하며, vendor SDK가 별도 해석.
...
struct spdk_ocssd_chunk_information_entry {
	...
	struct {
		 ...
		 * 의미: vector_write를 시작할 수 있는 후보. wp는 0.
		 * 값 범위: 1=free, 0=다른 상태. closed/open/offline과 상호 배타.
		 * 설정자: 디바이스(reset 완료 시 또는 초기 상태).
		 ...
		 * 와이어 위치: byte 0의 bit1.
		 * 의미: wp == cnlb. 이후 read만 가능, write는 SC_OUT_OF_ORDER 또는
		 * SC_WRITE_FAIL_CHUNK_EARLY_CLOSE 반환.
		 * 설정자: 디바이스(쓰기 완료 시 자동 전이, 또는 조기 종료).
		 ...
		 * 와이어 위치: byte 0의 bit2.
		 * 의미: 0 < wp < cnlb. 추가 sequential write 가능.
		 * 설정자: 디바이스(첫 vector_write로 free→open 전이).
		 ...
		 * 설정자: 디바이스(grown bad block 검출 시).
		 * 읽는 자: 호스트 FTL이 bad block table에 등록, 사용 풀에서 제외. */

		uint8_t reserved	: 4;
		/* [한국어] cs의 상위 4비트 예약. 디바이스 0, 호스트 무시. */
	} cs;
	...
	struct {
		 ...
		 * 설정자: 디바이스. 읽는 자: 호스트 FTL의 write 정렬 정책 분기.
		 * seq_write/rnd_write는 mutually exclusive. */
		 ...
		 * 설정자: 디바이스. 읽는 자: 호스트 FTL의 chunk 용량 계산기. */
	 ...
	 * 값 범위: 0~255 (vendor 정의 스케일, 보통 erase 횟수 기반).
	 * 설정자: 디바이스. 읽는 자: 호스트 FTL이 wear leveling 결정에 사용.
	 * geometry.wit과의 차이가 임계 초과면 chunk_notification.wit_exceeded
	 * 발생. */

	uint8_t reserved[5];
	/* [한국어] byte 3~7 예약. 0 채움. */
	 ...
	 * 설정자: 디바이스(geometry 결정적). 읽는 자: 호스트 FTL이
	 * vector_write/read의 LBA list를 빌드할 때 base 주소로 사용. */

	/** Number of blocks in chunk */
	uint64_t cnlb;
	/* [한국어] chunk 내 logical block 총 개수(=용량).
	 * 와이어 위치: byte 16~23.
	 * 값 범위: 보통 geometry.clba와 동일, 단 ct.size_deviate=1이면 다름.
	 * 설정자: 디바이스. 읽는 자: 호스트 FTL이 chunk full 판정(wp == cnlb)
	 * 및 sequential write 상한 검증에 사용. */
	 ...
	 * 설정자: 디바이스(vector_write 성공 시 자동 증가).
...
struct spdk_ocssd_chunk_notification_entry {
	 ...
	 * 값 범위: 1~UINT64_MAX. 설정자: 디바이스. 읽는 자: 호스트 FTL. */
	 ...
	 * 값 범위: 1~num_ns(0/0xFFFFFFFF은 통상 잘못된 통지).
	 * 설정자: 디바이스. 읽는 자: 호스트 FTL이 namespace별 캐시로 라우팅. */
	...
	struct {
		 ...
		 * 설정자: 디바이스. 읽는 자: 호스트 FTL의 wear/health 모니터. */
		...
		uint8_t rsvd : 3;
		/* [한국어] state byte 20의 bit5~7 예약. 0. */
		 ...
		 * 함을 시사. 설정자: 디바이스 wear monitor. 읽는 자: 호스트 FTL. */

		uint8_t rsvd2 : 7;
		/* [한국어] byte 21의 bit1~7 예약. 0. */
	} state;
	...
	struct {
		 ...
		 * 설정자: 디바이스. 읽는 자: 호스트 FTL의 통지 디스패처. */
		 ...
		 * 설정자: 디바이스. 읽는 자: 호스트 FTL이 chunk_information 항목 전체 갱신. */
		 ...
		 * 설정자: 디바이스. 읽는 자: 호스트 FTL의 PU-wide 정책 핸들러. */

		uint8_t rsvd : 5;
		/* [한국어] mask byte 22의 bit3~7 예약. 0. */
	} mask;

	uint8_t			rsvd[9];
	/* [한국어] byte 23~31 예약. spec 확장 placeholder. */
	 ...
	 * 설정자: 디바이스. 읽는 자: 호스트 FTL의 LBA 범위 갱신기. */

	uint8_t			rsvd2[30];
	/* [한국어] byte 34~63 예약. 64바이트 정렬을 맞추기 위한 패딩. */
};
...
struct spdk_ocssd_vector_cpl {
	 ...
	 * 실패했는지 식별 가능.
	 * 값 범위: bit 0~63 (LBA 인덱스 동일). 사용하지 않는 비트는 0.
	 * 설정자: 디바이스. 읽는 자: 호스트 FTL이 부분 실패 시 재시도 대상
	 * LBA 추출. */
	 ...
	 * 설정자: 디바이스. 읽는 자: SPDK 드라이버 + 호스트 FTL. */
};
...
enum spdk_ocssd_admin_opcode {
	 ...
	 * 페이지로 가져오는 admin 명령.
	 * 와이어 위치: SQE.opc=0xE2, NSID=대상 namespace, PRP1/PRP2=4KB 응답
	 * 버퍼. 결과는 spdk_ocssd_geometry_data로 캐스팅하여 해석.
...
enum spdk_ocssd_io_opcode {
	 ...
	 * 와이어: SQE.opc=0x90, cdw10/11=PPA list, cdw12.NLB=리셋 chunk 수-1.
	 ...
	 * 발급 주체: spdk_nvme_ocssd_ns_cmd_vector_reset(). */

	SPDK_OCSSD_OPC_VECTOR_WRITE	= 0x91,
	/* [한국어] Vector Chunk Write — 다수 LBA에 동시에 데이터 기록.
	 * 와이어: SQE.opc=0x91, cdw10/11=PPA list, PRP1/PRP2=write data,
	 ...
	 * 처리. 발급 주체: spdk_nvme_ocssd_ns_cmd_vector_write(). */

	SPDK_OCSSD_OPC_VECTOR_READ	= 0x92,
	/* [한국어] Vector Chunk Read — 다수 LBA에서 동시에 데이터 읽기.
	 * 와이어: SQE.opc=0x92, cdw10/11=PPA list, PRP1/PRP2=read 버퍼.
	 ...
	 * 와이어: SQE.opc=0x93, cdw10/11=dest PPA list, cdw14/15=src PPA list,
	 * cdw12.NLB=LBA 수-1.
...
enum spdk_ocssd_log_page {
	 ...
	 * 와이어: Get Log Page SQE.cdw10의 LID에 0xCA, 응답은
	 ...
	 * 와이어: Get Log Page SQE.cdw10의 LID에 0xD0, 응답은
	 * spdk_ocssd_chunk_notification_entry × N 배열.
	 * 트리거: 디바이스가 AER로 통지 → 호스트가 0xD0 폴링.
...
enum spdk_ocssd_feat {
	 ...
	 * 와이어: Set/Get Features SQE.cdw10의 FID에 0xCA.
	 * 의미: chunk_notification에서 어떤 종류의 미디어 이벤트(에러율 변동,
	 * WIT 초과 등)를 받을지 호스트가 활성/비활성 토글.
	 * 설정자: 호스트 FTL의 초기화 단계. 읽는 자: 디바이스 펌웨어가
	 * 어떤 통지를 발행할지 결정. */
};
 ...
 * 와이어 위치: SQE.cdw12의 최상위 비트(1U << 31).
